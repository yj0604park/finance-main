...
- STOCK 계좌인 경우 현재 보유 종목과 주식 거래 내역
...
- 검토 완료 취소(toggle off)는 거래 상세 페이지에서 처리한다
...
- 경고(warning) 표시 조건:
  ...
  - 거래 유형이 수입성(INCOME)인데 금액이 음수 → "수입 항목인데 음수입니다"
...
- **Retailer 목록 로딩 전략: 앱 레벨 캐싱**
  ...
  - relay 제한에 맞춰 100건씩 페이지네이션하며 전체 목록 로드
...
- 집계에서 제외되는 거래:
  ...
  - `type=STOCK` 거래 — 현금→증권 자산 이동이므로 소비 아님
...
- 실제 매수/매도(`StockTransaction`) 추가는 계좌 상세 페이지에서 처리
...
- **종목 목록 로딩 전략: retailer와 동일하게 앱 레벨 캐싱**
  - 폼 열 때마다 재로드 하지 않음
  - relay 제한에 맞춰 100건씩 페이지네이션하며 전체 목록 로드, Apollo 캐시 재사용
